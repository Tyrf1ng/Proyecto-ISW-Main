...
const NAVIGATION_BY_ROLE = {
  ...
  docente: [
    ...
    },
    {
      ...
      children: [
        {
          segment: "add_reservadocente",
          title: "Añadir Reservas",
          icon: <AddIcon />,
        },
        {
          segment: "ver_reservasdocentes",
          title: "Mis Reservas",
          icon: <EventIcon />,
        },
        {
...
const EXTRA_NAVIGATION_BY_ROLE = {
  alumno: {
    segment: "seleccionar_asignatura",
    title: "Seleccionar Asignatura",
    icon: <AssignmentIcon />,
    path: "/asignaturas",
  },
  docente: {
    segment: "seleccionar_curso",
...
const DashboardLayoutAccount = () => {
  ...
  const [session, setSession] = useState(() => {
    ...
    return usuarioGuardado
      ? { user: usuarioGuardado, role: usuarioGuardado.rol }
      : null;
  });
  ...
  const navigation = React.useMemo(() => {
    if (session?.role) {
      ...
      const roleNav = NAVIGATION_BY_ROLE[role] || [];
      const extraNav = EXTRA_NAVIGATION_BY_ROLE[role]
        ? [EXTRA_NAVIGATION_BY_ROLE[role]]
        : [];
      return [...extraNav, ...roleNav];
    }
    return [];
  }, [session]);
    ...
    return navigation.map((item) => (
      <div key={item.segment}>
        {item.children && item.children.length > 0 ? (
          <>
            <div
              className={`block p-2 text-gray-700 dark:text-gray-300 rounded`}
              aria-disabled="true"
            >
          ...
          <Link
            to={item.path || `/${item.segment.toLowerCase()}`}
            className="block p-2 text-gray-700 dark:text-gray-300 hover:bg-gray-200 dark:hover:bg-gray-700 rounded"
          >
            <div className="flex items-center space-x-2">
              {item.icon}
              <span className="text-sm font-medium">{item.title}</span>
            ...
            <div className="mt-6">
              <div className="flex items-center justify-between px-5 mt-6">
                ...
                <button
                  ...
                  className="text-gray-500 transition-colors duration-200 dark:text-gray-400 rtl:rotate-0"
                >
                  <svg
                  ...
                  >
                    <path
                      ...
                      d="M6.75 12h13.5M12 5.25l7.5 7.5-7.5 7.5"
                    />
                  </svg>
                </button>
              </div>
