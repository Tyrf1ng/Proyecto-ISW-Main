...
import { horarioBodyValidation } from "../validations/horarios.validation.js";
...
export async function createHorario(req, res) {
    try {
        ...
        const { error: bodyError } = await horarioBodyValidation.validateAsync({ hora_inicio, hora_fin });
...
export async function updateHorario(req, res) {
    try {
        const { id_horario } = req.params;
        const { body } = req;
        const { error: bodyError } = await horarioBodyValidation.validateAsync(body);
        if (bodyError) return handleErrorClient(res, 400, bodyError.message);
